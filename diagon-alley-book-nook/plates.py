...
PARTS_LIST = os.path.join(HERE, "docs", "03_PARTS_LIST.md")
...
def main():
    ...
    write_parts_list({r["id"] for r in json.load(open(os.path.join(OUT, "manifest.json")))
                      if B.needs_brim(r)})
...
# Group headings for the parts list, in the order the list presents them.
PARTS_LIST_GROUPS = [
    ("structure", "Chassis structure", "light grey, painted"),
    ("facade_L",  "Left facade",       "light grey, painted"),
    ("facade_R",  "Right facade",      "light grey, painted"),
    ("signs",     "Signs and ironwork", "light grey, painted"),
    ("props",     "Props",             "light grey, painted"),
    ("lighting",  "Lighting",          "natural / translucent for diffusers, black for holders"),
    ("case",      "Outer case",        "matte black, unpainted"),
    ("switch",    "Switch module",     "matte black, unpainted"),
    ("jigs",      "Bench tools",       "anything -- none of these go into the nook"),
]


def write_parts_list(brim):
    """docs/03_PARTS_LIST.md, from the manifest.

    It used to say "Generated from out/manifest.json" and be a hand-kept snapshot, which
    is worse than either. It was two generations stale by the time anyone read it -- 218
    parts and 1605 g against 185 and 1664 -- and it still listed the 35 sills, lintels
    and door frames that now print as part of their frames. A file that claims to be
    generated has to be.
    """
    ...
    os.makedirs(os.path.dirname(PARTS_LIST), exist_ok=True)
    with open(PARTS_LIST, "w") as f:
        w = f.write
        w("# Parts list\n\n")
        w("*Generated by `plates.py` from `out/manifest.json`. Do not edit by hand.*\n\n")
        w(f"**{len(rows)} printable parts, {sum(r['grams'] for r in rows):.0f} g of PLA.** "
          f"Every part is a single watertight solid, fits the {P.BED_X:.0f} mm build "
          "volume, and is exported already lying in its print orientation -- no part "
          "stands on end and none needs supports.\n\n")
        w(f"{len(brim)} of them want a 5 mm outer brim, marked **brim**. See "
          "`docs/05_PRINT_SETTINGS.md`.\n\n")
        seen = set()
        for key, title, filament in PARTS_LIST_GROUPS:
            group = sorted(by.get(key, []), key=lambda r: r["id"])
            if not group:
                continue
            seen.add(key)
            w(f"## {title} -- {len(group)} parts, {sum(r['grams'] for r in group):.0f} g\n\n")
            w(f"*Filament: {filament}*\n\n")
            w("| ID | Part | Print size (mm) | g | Notes |\n|---|---|---|---|---|\n")
            for r in group:
                bb = r["bbox"]
                note = r.get("note") or ""
                if B.is_cut(r):
                ...
                w(f"| `{r['id']}` | {r['name']} | {bb[0]:.0f} x {bb[1]:.0f} x {bb[2]:.0f} "
                  f"| {r['grams']:.1f} | {note} |\n")
            w("\n")
        for key in sorted(set(by) - seen):
            raise SystemExit(f"parts list: no heading for manifest group {key!r}")
