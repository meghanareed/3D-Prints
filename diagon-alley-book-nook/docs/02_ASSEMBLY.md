...
**This is resolved.** The envelope check now measures the placed bounding boxes from the
seating plane and the rear datum, and the chassis fits: 94.9 mm in a 95.6 mm cavity,
213.7 in 213.8, 197.6 in 197.8. `50_Outer_Left` to `59G_Foot_Pad` are off hold and are
on plates `08_case` to `08_case_4`.
...
`docs/05_PRINT_SETTINGS.md` is the same settings written out longhand, for printing from
the STL plates instead or on another machine. It is generated by `mf3.py` from the same
profile the projects carry, so the two cannot drift, and it includes the part-by-part
brim list.

**Why the brims are per-object and not left to Auto.** Auto looked at
`19C_L_L_Wall_Plaque` (15 mm² on the bed) and `13As_L_L_Window_A_Sill` (a sill 3 mm wide
under 7 mm of height) and decided neither needed one. Both came off the plate as
spaghetti. `build.needs_brim` decides instead, from the mesh in its print orientation: a
first layer under 25 mm²; or a part more than twice as tall as its footprint is narrow;
or more than 50 mm² of overhang over a quarter as much bed; or a part over 150 mm across
that is either under 6 mm tall or putting less than a quarter of its own footprint on the
bed. The last of those is the wall faces and ribs and the outer case panels — see *The
big flat parts need a brim* below.

**Outer only, never "outer and inner".** An inner brim lays brim inside every window
opening, where it is unreachable to pick out.

Every part that carries a brim also has **`[brim]`** in its object name, so the list is
visible in the slicer's object panel without opening anything.

> If you would rather not trust the per-object settings, or you are on a different
> slicer: set **Brim type "Outer brim only", 5 mm, in the GLOBAL tab** and leave it
> there for the whole kit. A brim does the big flat parts no harm, so a global setting
> is simpler than picking 65 parts out of 220 by hand.
>
> Setting it on the **Objects** tab applies it to the selected object only. A saved
> project checked here had `brim_type = outer_only` on one object and `auto_brim`
...
### The window panes are cut, not printed

All 25 of them. Every frame's rebate is `DIFFUSER_SLOT_T` deep and is documented as
taking vellum, acetate, PET or 1 mm acrylic as readily as a printed pane — cut sheet was
always an intended option, not a substitute. Acetate reads as glass; 0.8 mm of clear PLA
reads as fog.

`71A_Glazing_Cut_Template_A` on `13_bench_tools` is the template: a 216 × 106 × 1.6 mm
sheet with the outline of all 25 panes cut through it and each one labelled with its
part id. Lay it on the sheet material, run a knife round the inside of an opening, and
the pane drops into its frame. The outlines are taken from the pane **solids**, not from
their nominal width and height, so an arched window gives an arched opening and the
template cannot disagree with the part it is a template for. It replaces four hand-picked
sizes — 26 × 34, 34 × 32, 24 × 26, 20 × 40 — none of which was any of the 25 real sizes.

The panes stay in the manifest and the parts list, marked *cut from acetate*, because
they are still parts of the model and the template is generated from them. `build.is_cut`
is what keeps them off the plates and out of the 3MFs, and it is one definition that
`plates.py` and `mf3.py` both read.

**17 of the 25 are diffusers, not decoration.** Those windows have an LED bead behind
them and the pane is what turns a bright point into a lit window: `10B`, `11A`, `12A`,
`13A`, `13B`, `12B`, `13C`, `13E`, `14A`, `20E`, `21B`, `22A`, `24A`, `23A`, `23B`,
`23D`, `23G`. Vellum diffuses better than acetate on those. The other 8 are clear glass
and can be left out entirely if you would rather have open windows.

The bow window's `10Bd_L1_Bow_Window_Diffuser` is the same 0.8 mm clear insert but is
named a diffuser rather than glazing, so it is still printed. It belongs on the template
too.
...
|---|---|---|---|
| `00_CALIBRATE_FIRST` | 2 | 35 g | **print this one first and stop** |
| `01_CALIBRATE_JOINTS` | 2 | 67 g | **print this before the case** |
...
| `03_wall_rib_RIGHT` | 1 | 62 g | hidden; flat — **brim** |
...
| `09_facade_left` | 31 | 38 g | face DOWN, pegs UP |
| `10_facade_right` | 28 | 27 g | face DOWN, pegs UP |
| `11_signs_props` | 38 | 19 g | |
| `12_hardware` | 22 | 31 g | lighting and switch parts |
| `13_bench_tools` | 3 | 52 g | coupons, paint handles, ID card and the glazing cut template — **none of these go into the nook** |

**20 plates, 157 printed parts, 1688 g** — plus **25 window panes cut from sheet**, which
are parts of the model but not of any plate. 182 parts in total.

> This table is written by hand and drifts whenever the packing changes.
> `docs/04_PRINT_CHECKLIST.md` and `docs/03_PARTS_LIST.md` are generated by `plates.py`
...
1. **Brim type: outer brim only. Width 5 mm.** Not mouse ears — see below. The 3MF
   projects already carry this; on the STL plates, set it yourself.
2. **Bed at 60–65 °C**, not 55. The Bambu PLA default is fine for small parts and low
   for these. The projects carry a filament profile that does this — see
   `docs/05_PRINT_SETTINGS.md`.
...
Six parts are wide enough for this to be the reason they are on the brim list, rather
than being small or tippy:

| Part | Footprint | Why |
|---|---|---|
| `01_L_Wall_Face`, `02_R_Wall_Face` | 203 × 193 × 3.1 | a sheet with no height to resist curling |
| `01R_L_Wall_Rib`, `02R_R_Wall_Rib` | 203 × 197 × 10 | 10 mm tall but a skeleton — 5,900 mm² of first layer in strips two nozzles wide and 200 mm long |
| `50_Outer_Left`, `51_Outer_Right` | 200 × 214 × 5.2 | the largest footprint in the kit |

`build.needs_brim` catches all six, so they carry a brim in the 3MF projects and appear
in the list in `docs/05_PRINT_SETTINGS.md`. The rest of the list is the small and tippy
parts.

> **Plate spacing changed for this.** Parts used to sit 6 mm apart, which meant two 5 mm
> brims would merge into one raft — on the left facade plate, 22 of the 64 parts. A raft
> that peels takes everything on it with it. `plates.py` now spaces parts at
> `2 × brim + 1 = 11 mm`, so no two brims can touch, at the price of one more plate.
